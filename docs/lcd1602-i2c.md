...
Community nominal PCB ~**80 × 36 mm**, hole centers ~**75 × 31 mm**. **Measured on this unit** (bezel face only): **71 mm** wide × **24 mm** high. Full plate notes / CAD openings: **[front-panel.md](front-panel.md)**.
...
## Confirmed on this unit (lcd_hello bring-up)

| Item | Value |
|------|--------|
| Live 7-bit address | **0x27** (i2c_scan; only device on bus) |
| Pin map | **LiquidCrystal_I2C / HW-061:** P0=RS, P1=RW, P2=E, P3=BL (active high), P4–P7=D4–D7 |
| Working firmware | [`esp32_firmware/apps/lcd_hello`](../esp32_firmware/apps/lcd_hello) — write once, no backlight blink |
| Note | Chip silk is **PCF8574AT** (A-variant range is normally 0x38–0x3F); this board still ACKs at **0x27**. Prefer scan over silk. |
| Contrast | Blue pot on backpack — required if backlight is on but characters are missing or solid blocks |
| Bus | SDA **21**, SCL **22**; driver used ~20 kHz for 3.3 V host → 5 V backpack margin |

## Still open

- [ ] Whether onboard pull-ups alone are enough with multiple I²C modules (keypad + LCD)
